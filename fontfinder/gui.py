#!/usr/bin/env python3
"""
GUI font viewer using FontFinder as backend.
"""
...
class FontViewerApp:
    """Interface graphique pour visualiser les polices utilisant FontFinder."""
    ...
    def __init__(self, root: tk.Tk):
        self.root = root
        self.root.title("FontFinder GUI - Visualiseur de polices")
    ...
    def _setup_ui(self):
        ...
        glyph_check = ttk.Checkbutton(
            options_row,
            text="Filtrer polices compatibles (FontFinder)",
    ...
    def _apply_filter(self):
        """Applique le filtre de glyphes en utilisant FontFinder."""
        if not self.filter_glyphs.get():
            self.filtered_fonts = self.font_names.copy()
            return
        ...
        try:
            # Utiliser FontFinder pour le filtrage
    ...
    def _refresh_list(self):
        ...
        if self.filter_glyphs.get():
            info_text = f"FontFinder: {shown} / {total} polices compatibles avec le texte"
        else:
            info_text = f"FontFinder: {total} polices trouvées"
